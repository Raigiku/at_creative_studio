...
# --- 1. Always rebuild the binary. ---
#
# The Go binary embeds the static/ directory at compile time via
# //go:embed. To avoid the "I edited app.js and nothing changed"
# trap, we always rebuild before launching — no mtime check, no
# shortcut. The dev loop is "edit → start script → see the change".
